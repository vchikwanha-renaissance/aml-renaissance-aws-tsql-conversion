    ...
    BEGIN
        ...
        IF (var_Count = 1) THEN
            ...
            var_Query := 'SELECT COUNT(DISTINCT NotificationID) FROM (
                ...
                WHERE N.ToUserAccountID = ' || CAST(var_UserAccountID AS VARCHAR(30)) || ' AND N.ActionCode IS NULL AND OT.Name = ''QTIExport'' AND N.TypeCode = ''BLKR''
        ...
        ELSE
            ...
            var_Query := 'INSERT INTO t$assesslevel
                ...
                CASE WHEN EXISTS (SELECT 1 FROM t$permission WHERE objecttype = 'DOtherTypes' LIMIT 1) 
                     THEN '(A.LevelCode = ''D'' AND A.TypeCode <> ''B'') OR ' 
                     ELSE '' END ||
                ...
                CASE WHEN EXISTS (SELECT 1 FROM t$permission WHERE objecttype = 'COtherTypes' LIMIT 1) AND var_AccessLevelCode IN ('C', 'T') 
                     THEN '(A.LevelCode = ''C'' AND A.LevelOwnerID = ' || CAST(var_UserCampusID AS VARCHAR(30)) || ' AND A.TypeCode <> ''B'') OR ' 
                     ELSE '' END ||
            ...
            var_Query := 'SELECT Notification.ObjectID, Notification.ObjectTypeID, 
                TO_CHAR(dbo.GetDateTimeByTimezone(Notification.CreatedDate, ' || var_RequiredTZ::TEXT || '), ''MM/DD/YYYY'') AS CreatedDate, 
            ...
            ELSE
                IF (var_FromDate != '-1') THEN
                    ...
                    var_Query := var_Query || ' AND Notification.CreatedDate BETWEEN 
                        DATE_TRUNC(''day'', ''' || var_FromDate::DATE || '''::TIMESTAMP) AND 
                        (DATE_TRUNC(''day'', ''' || var_ToDate::DATE || '''::TIMESTAMP) + INTERVAL ''1 day'' - INTERVAL ''1 second'')';
            ...
            var_Query := 'SELECT Notification.ObjectID, Notification.ObjectTypeID, 
                ...
                TO_CHAR(Notification.ActionDate, ''MM/DD/YYYY'') AS ActionDate, '''''' AS empty1, Notification.AdditionalData, '''''' AS empty2 
            FROM Notification
